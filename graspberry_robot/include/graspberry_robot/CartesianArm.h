...
class CartesianArm : public GRASPberryArm {
	enum State {
		UNINITIALISED,
		POSITION,
		HOMING
    ...
    const std::vector<double> axis_ratio = {0.172, -0.100, 0.020}; //x, y, z axis ratios in metre/rev
    const std::vector<double> cob_id = {0x604, 0x606, 0x605}; //x, y, z axis ratios in metre/rev
    const std::vector<uint8_t> homing_mode = {0x6, 0x5, 0x6};
    ...
    virtual void zero() {
    	...
    	for (int i = 0; i < joint_controller.size(); i++) {
    		joint_controller[i]->ModeOfOperation(CANOpen::JMCController::ModeOfOperationValue::HomingMode);
	      	joint_controller[i]->Write(CANOpen::CiA402::Command::ZeroReturnMode, homing_mode[i]);//6 ccw?
