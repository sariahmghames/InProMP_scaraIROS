...
for lab in range(0,len(classa.keys())+1): # len(classa.keys()) = 19 without bkgd so lab = 0, ... 18
    ...
    if lab != 0:
        ...
        opend.io.write_point_cloud('./annotated_straw/straw{}.ply'.format(lab), pcdb)
    else:
...
# ...into the OctoMap native type by using conversions.h in the package octomap_ros... at v. first, convert PointCloud2 open3d format to ros format
