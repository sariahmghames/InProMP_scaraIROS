...
def push_promp(IC0, cluster_init, cond_pts, Ncond_pts, conf):
	Xee = list()
	Yee = list()
	Zee = list()
	...
	if len(cond_pts)/2 >= 2:
		t_cam2 = 1.5/(len(cond_pts)/2)
	elif (len(cond_pts)/2) < 2 and (len(cond_pts)/2) != 0:
		t_cam2 = 0.98/(len(cond_pts)/2)
	else:
		t_cam2 = 0.98
	print('t_cam2=', t_cam2)
	...
	mu_x_t1g1 = np.array([cluster_init[0], cluster_init[1], cluster_init[2]-0.1]) 
	...
	ax.scatter(cluster_init[0], cluster_init[1], cluster_init[2], s = 100, c='g', marker='o')
	...
	for k in range(0,len(mu_x_tfn)):
		...
		if k == 0 or k==1:
			tng1 = t_discrete[k+1] # Tune those for trajectory variation minimization
		else:
	...
	## Save data for testing
  	trajectories_Pushed = np.array([traj_Xee_condtng1, traj_Yee_condtng1,traj_Zee_condtng1])
	...
	if (len(Ncond_pts) != 0): # plot subpts that are not conditioned (by optimization)
		for i in range(len(Ncond_pts)):
			...
			ax.scatter(Ncond_pt[0], Ncond_pt[1], Ncond_pt[2], s = 100, c='g', marker='o')
			ax.scatter(np.repeat(Ncond_pt[0], 50), np.repeat(Ncond_pt[1], 50), np.linspace(Ncond_pt[2], Ncond_pt[2]+0.2, 50), c='g', marker='.')
	for l in range(0, len(mu_x_tfn)):
		...
		if len(j) == 0:
			...
			if (loop % 2 == 0):
				...
				ax.scatter(np.linspace(update_pose[0], root[0], 50), np.linspace(update_pose[1], root[1], 50), np.linspace(update_pose[2], root[2], 50), c='g', marker='.')
			loop = loop + 1
		else:
			...
			if l == 0:
				...
				if (loop % 2 != 0):
					...
					ax.scatter(np.linspace(update_pose[0], root[0], 50), np.linspace(update_pose[1], root[1], 50), np.linspace(update_pose[2], root[2], 50), c='g', marker='.')
				loop = loop + 1
			ax.scatter(np.linspace(mu_x_tf[0], mu_x_tfn[-1][0], 50), np.linspace(mu_x_tf[1], mu_x_tfn[-1][1], 50), np.linspace(mu_x_tf[2], mu_x_tfn[-1][2]+0.2, 50), c='g', marker='.')

	# ST_g1
	ax.scatter(traj_Xee_condt1g1, traj_Yee_condt1g1, traj_Zee_condt1g1, c='b', marker='.')
	ax.scatter(np.repeat(cluster_init[0], 50), np.repeat(cluster_init[1], 50), np.linspace(cluster_init[2], cluster_init[2]+0.2, 50), c='g', marker='.')
	##
...
def PushOpt_planner(bunch_xyz, bunch_col, bunch_orient, conf):
	...
	for pts in collection_ord['points']:
		...
		if (collection_ord['color'][ii] == True):
			...
			del lis[ind]
			goal_NN = clustering.rNN(lis,cluster_init, r = 0.05) # to add to paper for clustering pt clouds
			...
			push_promp(IC0, cluster_init, cond_pts, subset_Ncond, conf)
...
if __name__ == '__main__':
	...
	X_goal1 = np.array([0, 0.5, 0.41-0.15-0.016])
	X_goal2 = np.array([0.15, 0.5, 0.44-0.15-0.016])  
	X_goal3 = np.array([-0.15, 0.5, 0.44-0.15-0.016])  

	# neighbors of X_goal1
	goal1_n1 = np.array([X_goal1[0]+0.03, X_goal1[1], X_goal1[2]-0.03])  
	goal1_n2 = np.array([X_goal1[0]-0.03, X_goal1[1], X_goal1[2]-0.03]) 

	# neighbors of X_goal2
	goal2_n1 = np.array([X_goal2[0]+0.03, X_goal2[1], X_goal2[2]+0.03]) 
	goal2_n2 = np.array([X_goal2[0]-0.03, X_goal2[1], X_goal2[2]+0.03])  

	# neighbors of X_goal3
	goal3_n1 = np.array([X_goal3[0]+0.03, X_goal3[1], X_goal3[2]])  
	goal3_n2 = np.array([X_goal3[0]-0.03, X_goal3[1], X_goal3[2]]) 


	# colors
	X_goal1c = True #  is a mature fruit, red
	X_goal2c = True
	X_goal3c = True
	goal1_n1c = False
	goal1_n2c = False
	...
	X_goal1o = np.array([0,0,1]) 
	X_goal2o = np.array([0,0,1]) 
	X_goal3o = np.array([0,0,1]) 
	goal1_n1o = np.array([0,0,1]) 
	goal1_n2o = np.array([0,0,1]) 
	goal2_n1o = np.array([0,0,1]) 
	goal2_n2o = np.array([0,0,1])
	goal3_n1o = np.array([0,0,1]) 
	goal3_n2o = np.array([0,0,1])  



	print('Please enter which cluster configuration you want to simulate.')
	print('case 1 is configuration where NF are below goal.')
	print('case 2 is configuration where NF are above goal.')
	print('case 3 is configuration where NF are @ same level of goal.')
	conf = input()
	if conf == 1:
		bunch_xyz.append(X_goal1) 
		bunch_xyz.append(goal1_n1)
		bunch_xyz.append(goal1_n2)
		bunch_col.append(X_goal1c)
		bunch_col.append(goal1_n1c)
		bunch_col.append(goal1_n2c)
		bunch_orient.append(X_goal1o)
		bunch_orient.append(goal1_n1o)
		bunch_orient.append(goal1_n2o)
	elif conf == 2:
		bunch_xyz.append(X_goal2)
		bunch_xyz.append(goal2_n1)
		bunch_xyz.append(goal2_n2)
		bunch_col.append(X_goal2c)
		bunch_col.append(goal2_n1c)
		bunch_col.append(goal2_n2c)
		bunch_orient.append(X_goal2o)
		bunch_orient.append(goal2_n1o)
		bunch_orient.append(goal2_n2o)
	elif conf == 3:
		bunch_xyz.append(X_goal3) 
		bunch_xyz.append(goal3_n1)
		bunch_xyz.append(goal3_n2)
		bunch_col.append(X_goal3c)
		bunch_col.append(goal3_n1c)
		bunch_col.append(goal3_n2c)
		bunch_orient.append(X_goal3o)
		bunch_orient.append(goal3_n1o)
		bunch_orient.append(goal3_n2o)  
	else:
